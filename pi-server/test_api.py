...
class PiServerApiTests(unittest.TestCase):
    ...
    def test_pair_replaces_legacy_uuid_phone_tokens_for_stable_android_id(self):
        device_status = self.client.get("/device/status")
        self.assertEqual(device_status.status_code, 200)
        pair_code = device_status.json()["pair_code"]
        legacy_phone_id = str(uuid.uuid4())

        legacy_pair = self.client.post(
            "/pair",
            json={"phone_device_id": legacy_phone_id, "pair_code": pair_code},
        )
        self.assertEqual(legacy_pair.status_code, 200)
        legacy_token = legacy_pair.json()["auth_token"]
        self.assertEqual(self.client.get("/device/status").json()["paired_devices"], 1)

        stable_pair = self.client.post(
            "/pair",
            json={"phone_device_id": "android-0123456789abcdef", "pair_code": pair_code},
        )
        self.assertEqual(stable_pair.status_code, 200)
        self.assertEqual(self.client.get("/device/status").json()["paired_devices"], 1)

        old_token_sync = self.client.post(
            "/sync",
            headers={"Authorization": f"Bearer {legacy_token}"},
            json={"phone_device_id": legacy_phone_id, "last_sync_at": 0, "changed_bags": [], "changed_items": []},
        )
        self.assertEqual(old_token_sync.status_code, 401)

    def test_pair_keeps_other_stable_android_phone_tokens(self):
        device_status = self.client.get("/device/status")
        self.assertEqual(device_status.status_code, 200)
        pair_code = device_status.json()["pair_code"]
        ...
        second_pair = self.client.post(
            "/pair",
            json={"phone_device_id": "android-fedcba9876543210", "pair_code": pair_code},
        )
        self.assertEqual(second_pair.status_code, 200)
        self.assertEqual(self.client.get("/device/status").json()["paired_devices"], 2)
    ...
    def test_home_dashboard_is_single_bag_ui_with_polling_and_pair_code_card(self):
        ...
        self.assertIn('id="wifi-selected-signal-value"', body)
        self.assertIn('id="wifi-selected-signal-note"', body)
        self.assertIn('id="wifi-password-input"', body)
        self.assertIn('id="wifi-password-toggle"', body)
        self.assertIn(".wifi-password-toggle.active {", body)
        self.assertIn("-webkit-tap-highlight-color: transparent;", body)
        self.assertIn('id="wifi-refresh-button"', body)
        self.assertIn('id="wifi-hero-actions"', body)
        self.assertIn('id="wifi-inline-keyboard"', body)
        self.assertIn('id="wifi-inline-connect-button"', body)
        self.assertIn('data-wifi-key="1"', body)
        self.assertIn('data-wifi-keyboard-action="connect"', body)
        ...
        self.assertIn("passiveWifiRefreshIntervalMs", body)
        self.assertIn("settingsClockRefreshIntervalMs", body)
        self.assertIn("syncBackgroundLoops()", body)
        self.assertIn('document.addEventListener("visibilitychange"', body)
        self.assertLess(body.index("</main>"), body.index('<nav class="bottom-nav"'))
        self.assertIn("max-height: 240px;", body)
        self.assertIn("selectedWifiUsesDisconnectAction", body)
        self.assertIn("currentWifiModalView", body)
        self.assertIn("updateWifiSelectedCard", body)
        self.assertIn("wifiInlineKeyboardVisible", body)
        self.assertIn("button.textContent = wifiInlineKeyboardShift ? \"Lower\" : \"Upper\";", body)
        self.assertIn("wifiPasswordToggle.classList.toggle(\"active\", wifiPasswordVisible);", body)
        self.assertIn("function selectedWifiIsCurrentNetwork()", body)
        self.assertIn("wifiNetworkBadgeLabel", body)
        self.assertIn("showPasswordEntry", body)
        self.assertIn("handleWifiNetworkSelection(targetSsid);", body)
        self.assertIn('switchScreen("wifi", { restoreScroll: false });', body)
        self.assertIn("active_screen: activeScreen === \"wifi\" ? lastPrimaryScreen || \"dashboard\" : activeScreen", body)
        ...
        self.assertNotIn("wifiPasswordInput.scrollIntoView", body)
        self.assertIn("This Raspberry Pi stays display-only. Inventory updates appear here after the paired phone syncs them.", body)
        ...
        self.assertNotIn('id="wifi-close-button"', body)
        self.assertNotIn('wifi-dialog-title">Wi-Fi connection<', body)
        ...
        self.assertNotIn('data-keyboard-submit-target="wifi-connect-submit"', body)
        self.assertNotIn('data-keyboard-submit-label="Connect"', body)
        self.assertNotIn("Disconnect only if you want to leave this network.", body)
        self.assertNotIn("wifiInlineKeyboardShift && /^[a-z]$/i.test(rawText)", body)
        ...
        self.assertNotIn('id="scan-modal"', body)
        self.assertNotIn("scan-close-button", body)
        self.assertNotIn("scan-viewfinder", body)
        self.assertNotIn("USB scan preview is ready", body)
        self.assertNotIn('window.setInterval(updateSettingsClock, 1000);', body)
        self.assertNotIn('window.setInterval(refreshWifiStatus, refreshIntervalMs);', body)
